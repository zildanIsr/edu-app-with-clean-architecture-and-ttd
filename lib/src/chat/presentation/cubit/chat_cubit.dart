...
import 'package:education_app/src/chat/domain/usecase/get_groups.dart';
import 'package:education_app/src/chat/domain/usecase/get_messages.dart';
import 'package:education_app/src/chat/domain/usecase/get_user_by_id.dart';
import 'package:education_app/src/chat/domain/usecase/join_group.dart';
import 'package:education_app/src/chat/domain/usecase/leave_group.dart';
import 'package:education_app/src/chat/domain/usecase/send_message.dart';
...
class ChatCubit extends Cubit<ChatState> {
  ...
  Future<void> joinGroup({
    ...
    final result =
        await _joinGroup(JoinGroupParams(groupId: groupId, userId: userId));
    result.fold(
      (failure) => emit(ChatError(failure.errorMessage)),
  ...
  Future<void> leaveGroup({
    ...
    final result =
        await _leaveGroup(LeaveGroupParams(groupId: groupId, userId: userId));
    result.fold(
      (failure) => emit(ChatError(failure.errorMessage)),
