...
void main() {
  ...
  final tException = APIException(message: 'message', statusCode: 'statusCode');
    ...
    test(
      ...
      () async {
        ...
        expect(
          result,
          equals(
            Left<Failure, dynamic>(APIFailure.fromException(tException)),
          ),
        );
        verify(() => remoteDataSrc.clear('id')).called(1);
        verifyNoMoreInteractions(remoteDataSrc);
  ...
  group('getNotifications', () {
    test(
      ...
      () {
        ...
        expect(
          result,
          emits(
            Right<dynamic, List<Notification>>(
              notifications,
            ),
          ),
        );
    ...
    test(
      'should emit a [APIFailure] when call to remote source is '
      'unsuccessful',
      () async {
        when(() => remoteDataSrc.getNotifications()).thenAnswer(
