...
void main() {
  ...
  group('addVideos', () {
    test('should add the provide [Video] to the firestore', () async {
      ...
      expect(videoCollectionRef.docs.first.data()['tittle'], tVideo.title);
    ...
    test('should add the provided thumbnail to the storage if it is a file',
        () async {
      final thumbnailFile = File('assets/images/default_users.png');
      ...
      final thumbnailURL = await storage
          .ref()
          .child(
            'course/${tVideo.courseId}/videos/${savedVideo['id']}/thumbnail',
          )
          .getDownloadURL();
  ...
  group('getVideos', () {
    ...
    //it's difficult to simulate an error with the fake firestore, because if
    //doesn't throw any errors, so let just test that it returns an empty
