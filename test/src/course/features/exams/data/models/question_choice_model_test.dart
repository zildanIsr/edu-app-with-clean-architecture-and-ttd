...
void main() {
  ...
  group('QuestionChoiceModel', () {
    ...
    var map = jsonDecode(fixtures('exam.json')) as DataMap;
    ...
    group('fromUploadMap', () {
      test(
          'should return a valid [QuestionChoiceModel] when the JSON '
          'is not null', () async {
        final tmap = jsonDecode(fixtures('uploaded_exam_question_choice.json'))
            as DataMap;
        final result = QuestionChoiceModel.fromUploadMap(tmap);
        expect(result, tQuestionChoiceModel.copyWith(questionId: ''));
      });
    });
